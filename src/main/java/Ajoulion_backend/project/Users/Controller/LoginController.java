...
*/
    ...
    public ResponseEntity<?> updateInfo(@RequestHeader HttpHeaders header, @RequestBody UsersDto usersDto) throws Exception {
        Long userId = userService.loginCheck(header);

        userService.updateInfo(userId, usersDto);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Update Success");
    }

    @PatchMapping("/update/image")
    public ResponseEntity<?> updateImg(@RequestHeader HttpHeaders header, @RequestBody UsersDto usersDto) throws Exception {
        Long userId = userService.loginCheck(header);

        userService.updateImg(userId, usersDto);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Update Success");
    }
}
