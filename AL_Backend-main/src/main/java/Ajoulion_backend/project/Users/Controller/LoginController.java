...
public class LoginController {
    ...
    public ResponseEntity<?> donatorJoin(@RequestPart UsersDto userDto, @RequestPart(required = false) MultipartFile profile)  throws IOException {
        userDto.setCategory(0); // 0 : 기부자
        userService.join(userDto, profile);
        return ResponseEntity.status(HttpStatus.CREATED).body("success");
    }

    @PostMapping("/join/receiver")
    public ResponseEntity<?> receiverJoin(@RequestPart UsersDto userDto, @RequestPart(required = false) MultipartFile profile)  throws IOException {
